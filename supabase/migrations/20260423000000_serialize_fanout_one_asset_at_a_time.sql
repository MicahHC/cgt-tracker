...
-- Math: 108 companies × 12s = 21.6 minutes per weekly fan-out. With
...
--
-- This is a compatibility shim for Haiku tier 1. Once the user is on
-- Anthropic tier 2 (50 RPM Haiku), we can raise back to 10/batch with
-- a shorter stagger in a follow-up migration. Code-side behavior is
-- unchanged — signal-detection already handles arrays of any length.
...
DECLARE
  ...
  v_count         int  := 0;
  v_total         int;
  v_stagger_secs  int  := 12;  -- 60s / 5 RPM = 12s per call
  v_payload       jsonb;
BEGIN
