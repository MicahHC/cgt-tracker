...
export function assessLaunch(asset: LaunchAsset, now = new Date()): LaunchAssessment {
  const text = (asset.us_commercialization_window || '').trim();
  ...
  return { priority, latestDate, forecastOnly, reason: `${priority}: recorded launch window ends ${latestDate}${forecastOnly ? '; imported global forecast, U.S. timing requires validation' : '; recorded estimate, subject to approval'}` };
}
