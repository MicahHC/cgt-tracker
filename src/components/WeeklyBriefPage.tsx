        ...
        <div className="max-w-4xl mx-auto text-center">
          ...
          <div className="mt-10 flex items-center justify-center gap-6 md:gap-10 flex-wrap">
            ...
            {(abmIntel?.uniqueOnSite ?? 0) > 0 && (
              <>
                <div className="prestige-divider-vert hidden md:block" />
                <HeroMetric value={abmIntel!.uniqueOnSite} label="On-site accounts" sub="ABM" />
              </>
            )}
          </div>
        ...
        <section className="reveal reveal-delay-1">
          <header className="flex items-end justify-between gap-4 mb-6 flex-wrap">
            <div>
              ...
              <h2 className="prestige-section-title mt-3">Hot takeaways &amp; recommended actions</h2>
              <p className="text-sm text-slate-500 mt-2 max-w-xl leading-relaxed">
                Synthesized from site engagement data across On-Market and Late-Stage audiences. Closed-won accounts excluded.
              </p>
            </div>
          </header>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
            {/* Key takeaways */}
            {abmIntel.takeaways.length > 0 && (
              <div className="prestige-card p-6 flex flex-col gap-4">
                <div className="flex items-center gap-2">
                  <Sparkles className="w-4 h-4 text-teal-600 flex-shrink-0" />
                  <span className="text-xs font-semibold uppercase tracking-widest text-teal-700">Key takeaways</span>
                </div>
                {abmIntel.headline && (
                  <blockquote className="border-l-2 border-teal-300 pl-4 text-sm text-slate-700 italic leading-relaxed">
                    {abmIntel.headline}
                  </blockquote>
                )}
                <ul className="space-y-3">
                  {abmIntel.takeaways.map((t, i) => (
                    <li key={i} className="flex items-start gap-3">
                      <span className="mt-1 w-5 h-5 rounded-full bg-teal-100 text-teal-700 text-[10px] font-bold flex items-center justify-center flex-shrink-0">{i + 1}</span>
            ...
            {/* Recommended actions */}
            {abmIntel.actions.length > 0 && (
              <div className="flex flex-col gap-3">
                <div className="flex items-center gap-2 mb-1">
                  <TrendingUp className="w-4 h-4 text-slate-500 flex-shrink-0" />
                  <span className="text-xs font-semibold uppercase tracking-widest text-slate-500">Recommended actions</span>
                </div>
                {abmIntel.actions.map((action, i) => (
                  <div key={i} className={`prestige-card p-4 border-l-4 ${i === 0 ? 'border-teal-500' : i === 1 ? 'border-slate-300' : 'border-slate-200'}`}>
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        <div className="flex items-center gap-2 flex-wrap mb-1">
                          <span className={`text-[10px] font-bold uppercase tracking-widest px-2 py-0.5 rounded-full ${i === 0 ? 'bg-teal-100 text-teal-700' : 'bg-slate-100 text-slate-600'}`}>
                            P{action.priority}
                          </span>
                          <span className="text-sm font-semibold text-slate-900">{action.account}</span>
                        </div>
                        <div className="text-[13px] font-medium text-slate-800">{action.action}</div>
                        <div className="text-xs text-slate-500 mt-1 leading-relaxed">{action.why}</div>
                      </div>
                    </div>
                    <div className="mt-3 rounded-md bg-slate-50 border border-slate-100 px-3 py-2">
                      <span className="text-[10px] font-semibold uppercase tracking-widest text-slate-400">Next step · </span>
                      <span className="text-xs text-slate-600">{action.next_step}</span>
                    </div>
                  </div>
                ))}
              </div>
